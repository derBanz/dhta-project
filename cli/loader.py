...
class Loader:
    ...
    def parse(self):
        ...
        if self.model == "personnel":
            self.__parse_personnel(employees)
        if self.model == "stock":
    ...
    def __parse_personnel(self, employees):
        """Parse the personnel list."""
        Employee = self.__load_class("Employee")
        for employee in employees:
            self.objects.append(Employee(**employee))
            if employee.get('head_of'):
                self.__parse_personnel(employee['head_of'])
        return self.objects
